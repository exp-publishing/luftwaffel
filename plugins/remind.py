...
table = Table(
    ...
    Column('added_user', String(30, collation='NOCASE')),
...
def load_cache(async, db):
    ...
    for network, remind_time, added_chan, added_time, user, message in (yield from async(_load_cache_db, db)):
        reminder_cache.append((network, remind_time, added_chan, added_time, user, message))


def _load_cache_db(db):
    query = db.execute(table.select())
    return [(row["network"], row["remind_time"], row["added_chan"], row["added_time"], row["added_user"], row["message"]) for row in query]
